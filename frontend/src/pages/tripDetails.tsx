...
import { ArrowRight, CopyIcon, MapPin, Trash } from 'lucide-react';
...
function TripDetails() {
	...
	const { mutateAsync: createInvitation, isPending: invitationPending } =
		useCreateInvitation({ tripId: id });
	const { mutateAsync: deleteInvitation } = useDeleteInvitation({ tripId: id });
	const [eventOpen, setEventOpen] = useState(false);
	const [inviteOpen, setInviteOpen] = useState(false);
	const [displayInviteLink, setDisplayInviteLink] = useState<string | null>(
		null,
	);
	...
	useEffect(() => {
		console.log(`Trip has ` + invitations + ' invitations');
	});

	useEffect(() => {
		// eslint-disable-next-line react-hooks/set-state-in-effect
		setDisplayInviteLink(null);
	}, [inviteOpen]);
	...
	const invitationForm = useForm<CreateInvitationInput>({
		resolver: zodResolver(createInvitationInput) as never,
		defaultValues: {
	...
	const onSubmitInvitation = async (
		values: z.infer<typeof createInvitationInput>,
	) => {
		try {
										...
										>
												...
												<Controller
													...
													render={({ field, fieldState }) => (
														<Field data-invalid={fieldState.invalid}>
															<FieldLabel htmlFor="event_create_form_name">
																Event Name
															</FieldLabel>
															<Input
																{...field}
																id="event_create_form_name"
											...
											<FieldGroup>
												<Controller
													...
													render={({ field, fieldState }) => (
														<Field data-invalid={fieldState.invalid}>
															<FieldLabel htmlFor="event_create_form_description">
																Description
															</FieldLabel>
															<Input
																{...field}
																id="event_create_form_description"
																aria-invalid={fieldState.invalid}
											...
											<FieldGroup>
												<Controller
													...
													render={({ field, fieldState }) => (
														<Field data-invalid={fieldState.invalid}>
															<FieldLabel htmlFor="event_create_form_location_name">
																Location Name
															</FieldLabel>
															<Input
																{...field}
																id="event_create_form_location_name"
																aria-invalid={fieldState.invalid}
																placeholder="CDG Airport"
																autoComplete="off"
															/>
															{fieldState.invalid && (
																<FieldError errors={[fieldState.error]} />
													...
													render={({ field, fieldState }) => (
														<Field data-invalid={fieldState.invalid}>
															...
															<div className="flex gap-2">
																<Input
																	{...field}
																	id="event_create_form_location_lat"
																	aria-invalid={fieldState.invalid}
																	placeholder="Lat"
																	autoComplete="off"
																	onChange={(e) => {
																...
																<Input
																	{...field}
																	id="event_create_form_location_lng"
																	aria-invalid={fieldState.invalid}
																	placeholder="Lng"
																	autoComplete="off"
																	onChange={(e) => {
											...
											<FieldGroup>
												<Controller
													...
													render={({ field, fieldState }) => (
														<Field data-invalid={fieldState.invalid}>
															<FieldLabel htmlFor="event_create_form_start_time">
																Start Time
															</FieldLabel>
															<Input
																{...field}
																id="event_create_form_start_time"
																aria-invalid={fieldState.invalid}
																type="datetime-local"
																onChange={(e) => {
											...
											<FieldGroup>
												<Controller
													...
													render={({ field, fieldState }) => (
														<Field data-invalid={fieldState.invalid}>
															<FieldLabel htmlFor="event_create_form_end_time">
																End Time
															</FieldLabel>
															<Input
																{...field}
																id="event_create_form_end_time"
																aria-invalid={fieldState.invalid}
																type="datetime-local"
																onChange={(e) => {
																	const orig = e.target.value; // e.g. "2025-05-05T14:30"
								...
								<CardHeader>
									...
									<Dialog open={inviteOpen} onOpenChange={setInviteOpen}>
										...
										<DialogContent>
											...
											>
												<FieldGroup>
													<Controller
														...
														render={({ field, fieldState }) => (
															<Field data-invalid={fieldState.invalid}>
																<FieldLabel htmlFor="invitation_create_form_limit">
																	Number of Uses
																</FieldLabel>
																<Input
																	{...field}
																	id="invitation_create_form_limit"
																	aria-invalid={fieldState.invalid}
																	type="number"
																	autoComplete="off"
																/>
													...
													<Controller
														...
														render={({ field, fieldState }) => (
															...
															>
																<Checkbox
																	{...field}
																	id="invitation_create_form_organizer"
																	name={field.name}
																	checked={field.value}
																	onCheckedChange={field.onChange}
																/>
																<FieldLabel htmlFor="invitation_create_form_organizer">
																	Invite as Organizer?
																</FieldLabel>
															</Field>
														)}
												...
												<FieldGroup>
													<Controller
														...
														render={({ field, fieldState }) => (
															<Field data-invalid={fieldState.invalid}>
																<FieldLabel htmlFor="invitation_create_form_expiry_time">
																	Expires
																</FieldLabel>
																<Input
																	{...field}
																	id="invitation_create_form_expiry_time"
																	aria-invalid={fieldState.invalid}
																	type="datetime-local"
																	onChange={(e) => {
																		const orig = e.target.value; // e.g. "2025-05-05T14:30"
											...
											<DialogFooter
												className={displayInviteLink ? 'sm:justify-start' : ''}
											>
												{displayInviteLink ?
