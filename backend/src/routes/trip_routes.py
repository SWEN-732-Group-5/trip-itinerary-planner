...
async def update_organizers(trip_id: str, update_request: UpdateOrganizersRequest):
    ...
    result = await db.trips.update_one(
        {"trip_id": trip.id}, {"organizers": [*new_organizers], "guests": [*new_guests]}
    )
    if result.modified_count < 1:
        raise HTTPException(
...
async def create_event(trip_id: str, creation_request: CreateEventRequest):
    ...
    if len(creation_request.location_coords) != 2:
        raise HTTPException(
            status_code=400,
            detail=f"Must provide both a latitude and longitude coordinate!",
        )
...
async def update_event_location(
    trip_id: str, event_id: str, update_request: UpdateEventLocationRequest
):
    db = get_db_client().trip_itinerary_planner
    trip = await db.trips.find_one({"trip_id": trip_id})
    if trip is None:
