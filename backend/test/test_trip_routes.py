...
def make_mock_collection(fake_data: list[dict]):
    ...
    def fake_find(query):
        ...
        results = [
            d for d in fake_data if d["trip_id"] == trip_id and d["user_id"] == user_id
        ]
        cursor = MagicMock()
...
def make_invitation_dict(
    invitation_id: str = "invitation1",
    trip_id: str = "trip1",
...
def test_get_trip_success():
    ...
    with patch("src.main.get_db_client") as mock_main_db_client_fn, patch(
        ...
        try:
            with TestClient(app) as client:
                response = client.get(
                    "/api/trips/trip1", headers={"session_token": "fake_token"}
                )
                assert response.status_code == 200
...
def test_get_trip_not_found():
    ...
    with patch("src.main.get_db_client") as mock_main_db_client_fn, patch(
        ...
        try:
            with TestClient(app) as client:
                response = client.get(
                    "/api/trips/trip999", headers={"session_token": "fake_token"}
                )
        ...
        try:
            with TestClient(app) as client:
                ...
                response = client.post(
                    "/api/trips", json=payload, headers={"session_token": "fake_token"}
                )
...
def test_update_trip_success():
    ...
    trips_collection.update_one = AsyncMock(
        return_value=make_update_result(updated_trip)
    )
    ...
    with patch("src.main.get_db_client") as mock_main_db_client_fn, patch(
        ...
        try:
            with TestClient(app) as client:
                ...
                trips_collection.delete_one.assert_awaited_once_with(
                    {"trip_id": "trip1"}
                )
        finally:
            app.dependency_overrides.clear()
...
def test_create_event_success():
    ...
    trips_collection.update_one = AsyncMock(
        return_value=make_update_result(None, modified_count=1)
    )
...
def test_update_event_success():
    ...
    trips_collection.find_one = AsyncMock(
        side_effect=[trip, {**trip, "events": [updated_event]}]
    )
...
def test_update_event_location_success():
    ...
    result_trip["events"] = [updated_event]
    trips_collection.update_one = AsyncMock(
        return_value=make_update_result(result_trip)
    )
...
def test_update_event_end_location_success():
    ...
    trips_collection.update_one = AsyncMock(
        return_value=make_update_result(
            {**make_trip_dict(), "organizers": ["user1"], "events": [updated_event]}
        )
    )
    ...
    with patch("src.main.get_db_client") as mock_main_db_client_fn, patch(
        ...
        try:
            with TestClient(app) as client:
                ...
                assert data["events"][0]["end_location"]["gps_position"] == [
                    42.0,
                    -74.0,
                ]
                trips_collection.update_one.assert_awaited_once()
...
def test_delete_event_success():
    ...
    trips_collection.update_one = AsyncMock(
        return_value=make_update_result(
            {**make_trip_dict(), "organizers": ["user1"], "events": []}
        )
    )
...
def test_update_trip_not_found():
    ...
    trips_collection.find_one = AsyncMock(return_value=None)
    trips_collection.update_one = AsyncMock(
        return_value=make_update_result(None, modified_count=0)
    )
    ...
    with patch("src.main.get_db_client") as mock_main_db_client_fn, patch(
        ...
        try:
            with TestClient(app) as client:
                ...
                assert response.status_code == 404
                assert (
                    "Could not find trip trip999 to update" in response.json()["detail"]
                )
...
def test_delete_trip_not_found():
    trips_collection = MagicMock()
    trips_collection.find_one = AsyncMock(return_value=None)
    trips_collection.delete_one = AsyncMock(
        return_value=make_delete_result(deleted_count=0)
    )
    ...
    with patch("src.main.get_db_client") as mock_main_db_client_fn, patch(
        ...
        try:
            with TestClient(app) as client:
                ...
                assert response.status_code == 404
                assert (
                    "Could not find trip trip999 to delete" in response.json()["detail"]
                )
    ...
    with patch("src.main.get_db_client") as mock_main_db_client_fn, patch(
        ...
        try:
            with TestClient(app) as client:
                ...
                assert response.status_code == 404
                assert (
                    "Could not find trip trip999 to add an event to"
            ...
            with TestClient(app) as client:
                response = client.put(
                    ...
                    json={
                        ...
                        "location_type": "food",
                        "location_coords": [40.0, -74.0],
                        "start_time": "2025-03-02T12:00:00Z",
                        "end_time": "2025-03-02T13:00:00Z",
                        "image_urls": [],
                    },
                    headers={"session_token": "fake_token"},
                )
...
def test_update_event_event_not_found():
    ...
    trips_collection.update_one = AsyncMock(return_value=mock_result)
    ...
    with patch("src.main.get_db_client") as mock_main_db_client_fn, patch(
        ...
        try:
            with TestClient(app) as client:
                ...
                assert response.status_code == 404
                assert (
                    "Could not find event event999 in trip trip1"
                    in response.json()["detail"]
                )
    ...
    with patch("src.main.get_db_client") as mock_main_db_client_fn, patch(
        ...
        try:
            with TestClient(app) as client:
                ...
                assert (
                    "Found trip trip1 but failed to update it"
                    in response.json()["detail"]
                )
...
def test_update_event_update_fails():
    ...
    trips_collection.update_one = AsyncMock(
        return_value=make_update_result(
            {**make_trip_dict(), "organizers": ["user1"], "events": [existing_event]},
            modified_count=0,
        )
    )
...
def test_update_organizers_update_fails():
    ...
    trips_collection.update_one = AsyncMock(
        return_value=make_update_result(
            {**make_trip_dict(), "organizers": ["user1"], "guests": ["user2"]},
            modified_count=0,
        )
    )
...
def test_create_trip_invitation_success():
    ...
    invitations_collection.find = MagicMock(
    ...
    with patch("src.main.get_db_client") as mock_main_db_client_fn, patch(
        ...
        try:
            with TestClient(app) as client:
                ...
                assert response.status_code == 404
                assert (
                    "Could not find trip trip1 to invite users to"
                    in response.json()["detail"]
                )
    ...
    with patch("src.main.get_db_client") as mock_main_db_client_fn, patch(
        ...
        try:
            with TestClient(app) as client:
                ...
                assert (
                    "Only organizers can create invitations for a trip"
                    in response.json()["detail"]
                )
    ...
    with patch("src.main.get_db_client") as mock_main_db_client_fn, patch(
        ...
        try:
            with TestClient(app) as client:
                ...
                assert response.status_code == 404
                assert (
                    "Could not find trip trip1 to get invitations for"
                    in response.json()["detail"]
                )
    ...
    with patch("src.main.get_db_client") as mock_main_db_client_fn, patch(
        ...
        try:
            with TestClient(app) as client:
                ...
                assert response.status_code == 403
                assert (
                    "Only organizers can view invitations for a trip"
                    in response.json()["detail"]
                )
...
def test_accept_trip_invitation_success_as_guest():
    ...
    invitations_collection = MagicMock()
    invitations_collection.find_one = AsyncMock(return_value=invitation)
    invitations_collection.update_one = AsyncMock(
        return_value=make_update_result(None, modified_count=1)
    )
    ...
    with patch("src.main.get_db_client") as mock_main_db_client_fn, patch(
        ...
        try:
            with TestClient(app) as client:
                ...
                assert response.status_code == 400
                assert (
                    "has already been used the maximum number of times"
                    in response.json()["detail"]
                )
    ...
    with patch("src.main.get_db_client") as mock_main_db_client_fn, patch(
        ...
        try:
            with TestClient(app) as client:
                ...
                assert response.status_code == 404
                assert (
                    "Could not find trip trip1 to accept invitation for"
                    in response.json()["detail"]
                )
    ...
    with patch("src.main.get_db_client") as mock_main_db_client_fn, patch(
        ...
        try:
            with TestClient(app) as client:
                ...
                assert response.status_code == 404
                assert (
                    "Could not find invitation invalid_invitation for trip trip1"
                    in response.json()["detail"]
                )
    ...
    with patch("src.main.get_db_client") as mock_main_db_client_fn, patch(
        ...
        try:
            with TestClient(app) as client:
                ...
                assert response.status_code == 404
                assert (
                    "Could not find trip trip1 to delete invitation from"
                    in response.json()["detail"]
                )
    ...
    with patch("src.main.get_db_client") as mock_main_db_client_fn, patch(
        ...
        try:
            with TestClient(app) as client:
                ...
                assert response.status_code == 403
                assert (
                    "Only organizers can delete invitations for a trip"
                    in response.json()["detail"]
                )
    ...
    with patch("src.main.get_db_client") as mock_main_db_client_fn, patch(
        ...
        try:
            with TestClient(app) as client:
                ...
                assert response.status_code == 403
                assert (
                    "Only organizers can remove users from a trip"
                    in response.json()["detail"]
                )
